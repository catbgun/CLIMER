...
Packages <- c("ggplot2", "lubridate", "scales", "plyr", "dplyr", "Rmisc", "ggpubr", "rstatix")
...
```{r Boxplot}
ggplot(dx, aes(x=Size, y= sUVa))+
  geom_boxplot()
  facet_grid(.~Position)
 
LMW <- subset(dx, Size=="LMW") 
HMW <- subset(dx, Size=="HMW") 
ggplot(HMW, aes(x=Position, y= THg_DOC))+
  geom_boxplot()
  facet_grid(.~Position)

wilcox.test(THg_DOC ~ Position, data=LMW)
...
ggplot(dx, aes(sUVa, THg))+
  geom_point(aes(shape=(factor(Size)), colour=interaction(Position, Size)),fill="black", size=6)+
  ...
  labs(y = "Hg (ng/L)", x="sUVa254" )+
    scale_colour_manual(values = c("#0072B2", "#CC79A7", "#bdd4ff", "#e1b0cb"))
...
#Rate.DOC.ave
ggplot(b3, aes(x=Fraction, y= Rate.DOC.ave, fill=Fraction))+
  geom_boxplot()+
  facet_grid(.~Lake)+
  scale_y_reverse() 
...
#Relative biodeg FIGURE 4
#rel_biodeg <- 
ggplot(b3, aes(x=Fraction, y= Rel.rate.DOC))+
  geom_boxplot(aes(fill=interaction(Lake, Fraction)))+
  facet_grid(.~Lake)+
  scale_fill_manual(values = c("#0072B2", "#CC79A7", "#bdd4ff", "#e1b0cb"))+
  theme_bw()+
  theme(axis.text.y = element_text(size= 18, colour="black"),
        axis.title.x = element_blank(),
        axis.title.y = element_text(size = 20, margin=margin(0,20,0,0)),
        strip.text.x = element_text(size = 22),
        axis.text.x = element_text(angle=0, hjust=0.5, vjust=0, size = 18, colour="black"),
        plot.title=element_text(size=30, face="bold"),
        legend.text=element_blank(),
        legend.title = element_blank())+
  labs(y = "DOC biodegradation rate (%)", x="Size fraction" )+
  scale_x_discrete(labels = c("LMW" = "Low\n molecular\n weight", "HMW" = "High\n molecular\n weight"), limits=rev)

  scale_x_discrete(limits=rev)


ggsave(filename = "output/rel_biodeg.png",
       plot = rel_biodeg,
...
b2_sum <- select(b2, "group", "Rate_C_mg_ave", "Rate.DOC.ave")
bs_sum <- na.omit(b2_sum)

class(bs_sum$Rate.DOC.ave)


ss <- bs_sum %>%
  group_by(group) %>%
  summarise(mean = mean(Rate_C_mg_ave), sd=sd(Rate_C_mg_ave), min=min(Rate_C_mg_ave), max=max(Rate_C_mg_ave))

sss <- bs_sum %>%
  group_by(group) %>%
  summarise(mean = mean(Rate.DOC.ave), sd=sd(Rate.DOC.ave), min=min(Rate.DOC.ave), max=max(Rate.DOC.ave))


sum_nx <- bs_sum %>% 
  group_by(group) %>% 
  length(dplyr::across(.cols  = where(is.numeric)))

bs_sum %>% count(Rate_C_mg_ave)
s <- summarise(bs_sum, Rate_C_mg_ave, Rate.DOC.ave, .by = NULL, .groups = group)
```
...
outlet <- subset(b3, Lake == "Outlet")
out2 <- subset(outlet, sUVa > 2)

ggplot(b3, aes(sUVa, Rate.DOC.ave))+
  geom_point(aes(shape=factor(Fraction), colour=interaction(Lake, Fraction)),fill="black", size=5)+
  geom_smooth(method = "lm", se = FALSE, colour="black")+
  stat_regline_equation(label.x=0.35, label.y=0.0098) +
  ...
  theme(axis.text.y = element_text(size= 12, colour="black"),
        ...
        legend.text=element_blank(),
        legend.title = element_blank())+
  stat_cor(method = "pearson", label.x = 0.30, label.y = 0.011)+
  scale_colour_manual(values = c("#0072B2", "#CC79A7", "#bdd4ff", "#e1b0cb"))+
  scale_y_reverse()+
  theme_bw()+
  labs(y= "DOC biodegradation rate (mg C/mg C)", x="sUVa-254 (L/mg C m)")+
  facet_grid(.~Lake)



ggsave(filename = "output/biodeg_suva.png",
       plot = biodeg_suva,
...
ggplot(dx, aes(DOC_UiO, THg))+
  ...
  theme(axis.text.y = element_text(size= 22),
        ...
        legend.text=element_text(size=26),
        legend.title = element_blank())+
  labs(y = "Hg (ng/L)", x="DOC (mg/L)" )+
  facet_grid(. ~ factor(Position))+
  scale_colour_manual(values = c("#0072B2", "#CC79A7", "#bdd4ff", "#e1b0cb"))



#FOR manuscript, TOC and Hg in grab samples
Hg_TOC <- ggplot(dx, aes(DOC_UiO, as.numeric(THg)))+
  geom_point(aes(shape=factor(Size), colour=interaction(Position, Size)),fill="black", size=5)+
  geom_smooth(method = "lm", se = FALSE, colour="black")+
  stat_regline_equation(label.x=0.35, label.y=5.5) +
  ...
  theme(axis.text.y = element_text(size= 14),
        ...
        legend.text=element_blank(),
        legend.title = element_blank())+
  stat_cor(method = "pearson", label.x = 0.30, label.y = 5.8)+
  labs(x="DOC (mg/L)", y="Hg (ng/L)")+
  facet_grid(.~Position)+
  scale_colour_manual(values = c("#0072B2", "#CC79A7", "#bdd4ff", "#e1b0cb"))

ggsave(filename = "output/Hg_TOC_corr.png",
       plot = Hg_TOC,
       width = 7, height = 5, units = "in")
