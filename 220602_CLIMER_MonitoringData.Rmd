...
A1_out <- ts(new_ts_out, frequency=365) #TOC data outlet
A1_inl <- ts(new_ts_in, frequency=365)  #TOC data inlet

B1_out <- ts(new_ts_out1, frequency=365) #Hg data outlet
B1_inl <- ts(new_ts_in1, frequency=365)  #Hg data inlet

library(stats)
#decomp <- stl(A1_out[,5], s.window = "periodic") #date is not on x axis
res1<- (stl(ts(A1_out[,5],frequency=365,start=c(2016,2 )),s.window="per",robust=TRUE) )#fixes date
res2<- (stl(ts(A1_inl[,5],frequency=365,start=c(2016,31)),s.window="per",robust=TRUE) )#fixes date

res3<- (stl(ts(B1_out[,5],frequency=365,start=c(2016,31 )),s.window="per",robust=TRUE) )#fixes date
res4<- (stl(ts(B1_inl[,5],frequency=365,start=c(2016,31)),s.window="per",robust=TRUE) )#fixes date
...
plot(res1$seasonal, res2$seasonal) 

res2


#the following can possibly be deleted: 

# Perform additive decomposition
decomposed_TOC_out <- decompose(A1_out[,5], type = "multiplicative")
decomposed_TOC_in <- decompose(A1_inl[,5], type = "multiplicative")

decomposed_Hg_out <- decompose(B1_out[,6], type = "multiplicative")
decomposed_Hg_in <- decompose(B1_inl[,6], type = "multiplicative")

#stl_decomposed_data <- stl(A1[,2], s.window = "periodic")
#plot(stl_decomposed_data)
class(decomposed_TOC_in)

#We need the date info back, and to correlate the seasonal/trend lines. 
decomposed_TOC_in

plot(decomposed_TOC_in)
title("Langtjern TOC inlet")

plot(decomposed_TOC_out)
title("Langtjern TOC outlet")

plot(decomposed_Hg_out)
title("Langtjern Hg outlet")

plot(decomposed_Hg_in)
title("Langtjern Hg inlet")

plot(decomposed_TOC_out$x, decomposed_TOC_in$x)  
